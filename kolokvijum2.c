...
int* izdvoj(int** mat, int n, int m, int (*fun)(int)) {
    duzina = 0;
    for(int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (fun(mat[i][j])) {
    ...
    int* rez = (int*)malloc(duzina*sizeof(int));
    int k = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            if (fun(mat[i][j])) {
...
int ukupna_cena(int** mat, int n, int m, int* (*izdv_fun)(int**, int, int, int (*fun1)(int)), int(*cmp_fun)(int), int cena) {
    int* izdvojeni = izdv_fun(mat, n, m, cmp_fun);
    int rez = 0;
    for (int i = 0; i < duzina; i++) {
        rez += cena * izdvojeni[i];
    }
    free(izdvojeni);
    return rez;
}
